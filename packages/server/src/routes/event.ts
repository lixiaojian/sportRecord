...
import { createEventSchema, updateEventSchema } from '@sport-record/shared';
...
eventRouter.get('/', optionalAuth, async (req, res) => {
  const { page, pageSize, skip, take } = parsePagination(req.query);
  const { list, total } = await service.list(req.user?.id, skip, take);
  success(res, paginated(list, total, { page, pageSize }));
});

eventRouter.post('/', authenticate, validate(createEventSchema, 'body'), async (req, res) => {
  const item = await service.create(req.body, req.user!.id);
  success(res, item, 201);
});

eventRouter.get('/:id', optionalAuth, async (req: Request<{ id: string }>, res) => {
...
eventRouter.delete('/:id', authenticate, async (req: Request<{ id: string }>, res) => {
